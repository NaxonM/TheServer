...
def load_video_attributes(video):
    ...
    try:
        if isinstance(video, ph_Video):
            ...
            if hasattr(video, 'fetch'): video.fetch()
            title = video.title
            author = getattr(video, 'author', None) or getattr(video, 'performer', None) or "N/A"
            length = getattr(video, 'length', 0)
            tags = getattr(video, 'tags', [])
            publish_date = getattr(video, 'publish_date', "N/A")
            thumbnail = getattr(video, 'thumbnail', None)
            
            # Enhanced quality fetching for Spankbang - uses Quality enum
            from spankbang_api.spanbkang_api import Quality as sp_Quality
            qualities = ['best', '1080p', '720p', '480p']  # Common resolutions as fallback
            try:
                # Try to get qualities from API-specific method
                qualities_list = video.get_available_qualities() if hasattr(video, 'get_available_qualities') else []
                if qualities_list:
                    qualities = [str(q) for q in qualities_list]
            except Exception as e:
                logger.warning(f"Quality extraction failed for Spankbang: {e}")
                # Fallback to enum values converted to strings
                if hasattr(sp_Quality, 'BEST'):
                    qualities = [sp_Quality.BEST.value, sp_Quality.HIGH.value, sp_Quality.MEDIUM.value, sp_quality.LOW.value]

        elif isinstance(video, (mv_Video, xh_Video, yp_Video)):
            # Generic handling for other providers
            if hasattr(video, 'fetch'): video.fetch()
            title = getattr(video, 'title', "N/A")
            author = getattr(video, 'author', "N/A")
            length = getattr(video, 'length', 0)
            tags = getattr(video, 'tags', [])
            publish_date = getattr(video, 'publish_date', "N/A")
            thumbnail = getattr(video, 'thumbnail', None)
        ...
        if not qualities:
            ...
            if isinstance(video, ep_Video):
                ...
                qualities = ['best', '1080p', '720p', '480p']  # Spankbang resolutions
            else:
                qualities = ['best', '1080p', '720p', '480p', '360p']  # Default resolution format
        ...
        return {
            ...
            "tags": [], "publish_date": "N/A", "thumbnail": None, "qualities": ['best', '1080p', '720p', '480p', '360p']
        }
