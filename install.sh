#!/bin/bash
#
# This script deploys or uninstalls the File Proxy System.
# It's designed to be run via curl:
# curl -sSL https://raw.githubusercontent.com/NaxonM/TheServer/main/install.sh | bash
#
set -e

# --- Static Configuration ---
# These variables define the installation environment and default settings.
PROJECT_DIR_NAME="file-proxy-system"
PROJECT_DIR="$HOME/$PROJECT_DIR_NAME"
PERSISTENT_DATA_DIR="$HOME/.${PROJECT_DIR_NAME}-data"
REPO_URL="https://github.com/NaxonM/TheServer.git"
ENV_FILE=".env"
...
show_usage() {
    ...
    echo
    echo "Commands:"
    echo "  (no command)     Deploys the 'main' branch or updates the existing installation."
...
handle_config() {
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        read -p "Enter the domain name for your dashboard (e.g., files.example.com): " config_domain
...
install_system() {
    log "--- Starting File Proxy System Deployment/Update ---"
    ...
    if [ -d "$PROJECT_DIR" ] && [ -d "$PROJECT_DIR/.git" ]; then
        ...
        log "[1/4] Updating application source to branch '$REPO_BRANCH'..."
        ...
        mkdir -p "$PROJECT_DIR"
        cd "$PROJECT_DIR"
        log "[1/4] Fetching application source from branch '$REPO_BRANCH'..."
        fetch_source_code
    else
    ...
    if [ -d "src/deploy" ] && [ -f "src/deploy/docker-compose.yml" ]; then
        log "New compose files found in src/deploy, moving to project root..."
        mv src/deploy/docker-compose*.yml .
        log_success "Docker Compose files moved."
    else
        # On subsequent runs, these files will already be in the root.
        log "Compose files already in project root. Skipping move."
    fi
    ...
    if [ "$DEPLOYMENT_TYPE" = "domain" ]; then
        ...
        echo -e "  ${C_YELLOW}Access your dashboard at: http://${DOMAIN}:8000${C_RESET}"
    fi
    echo
...
case "$1" in
    uninstall)
        ...
        ;;
    install|"")
        # Use default 'main' branch for 'install' or no argument
        install_system
        ;;
    *)
